...
import { isTodoFile, parseDocument } from '../../core/parser';
...
export const userCompletionProvider: vscode.CompletionItemProvider = {
    async provideCompletionItems(document, position) {
        ...
        const nextChar =
            position.character < lineText.length ? lineText.charAt(position.character) : '';
    ...
    async provideCompletionItems(document, position) {
        ...
        return parsed.projectDefinitions.map((p) => {
            ...
            const replaceEnd = new vscode.Position(
                position.line,
                position.character + swallowAutoClose
            );
