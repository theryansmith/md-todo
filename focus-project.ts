...
import { getFocusProject, setFocusProjectState } from './state';
...
export function initFocusProjectStatusBar(context: vscode.ExtensionContext): void {
    ...
    projectFocusStatusBarItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 97);
...
export function refreshFocusProjectStatusBar(editor: vscode.TextEditor | undefined) {
    if (!projectFocusStatusBarItem) { return; }
...
export async function setFocusProject(): Promise<void> {
    ...
    const picks: ProjectPick[] = [
        { label: '$(circle-slash) Clear focus', description: 'Show all projects', projectName: undefined },
    ...
    const placeHolder = current ? `Currently focused on [${current}]` : 'Select a project to focus on (or clear)';
