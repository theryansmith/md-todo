...
- Internal: the four focus modules (user/tag/project/activity) are now one generic `FocusDimension` engine plus four small descriptors, the activity reports moved to a dedicated reports module, and workspace-state access is typed (Phase 3d). Status-bar items, pickers, focus dimming, and every command ID are pinned by characterization tests and unchanged; no user-facing behavior changes.

### Fixed

- Todo checkboxes, `#tags`, `@mentions`, dates, `## Section` headers, and definition lines inside **fenced code blocks** (``` or ~~~) and **HTML comment blocks** (`<!-- -->`) are no longer treated as real content. Previously a code sample containing `- [ ] example` showed up as a live todo in the tree views and reports, and tokens inside fences/comments were syntax-highlighted and counted. Lines inside fences and comments are now skipped entirely by the parser and by the editor decorations. (Inline code spans on normal lines are unchanged — and date `` `+…` ``/`` `✓…` `` and project `` `[…]` `` tokens still require their backticks by design.)
...
- `MD Todo: Show File History` no longer builds shell command strings: git is now invoked with `execFile` and verbatim arguments, so todo files in paths containing spaces, `$`, backticks, or quotes work instead of breaking (or being shell-expanded).
