...
- [ ] Zero copies of the "Not a todo file" guard outside `vscode/guards.ts`
- [ ] `getEffectiveEditor` and `EffectiveEditorContext` deleted
...
- [ ] Every decoration type, status-bar item, tree view, and cache is disposed/cleared via `context.subscriptions`/`CacheRegistry`; `deactivate` verified leak-free
