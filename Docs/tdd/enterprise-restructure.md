...
- [x] Every decoration type, status-bar item, tree view, and cache is disposed/cleared via `context.subscriptions`/`CacheRegistry`; `deactivate` verified leak-free (note: the tree providers' internal debounce timers are the one remaining softness — revisit in Phase 3c when the providers are genericized)
